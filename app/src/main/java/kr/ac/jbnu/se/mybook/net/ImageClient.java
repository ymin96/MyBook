package kr.ac.jbnu.se.mybook.net;

import com.loopj.android.http.AsyncHttpClient;
import com.loopj.android.http.JsonHttpResponseHandler;

import java.io.UnsupportedEncodingException;
import java.net.URLEncoder;

public class ImageClient {
    private static final String API_BASE_URL = "https://dapi.kakao.com/";
    private static final String API_KEY="KakaoAK 60803234e1c07b098ac28d8424af9b24";

    private AsyncHttpClient client;

    public ImageClient() {
        this.client = new AsyncHttpClient();
    }

    private String getApiUrl(String relativeUrl) {
        return API_BASE_URL + relativeUrl;
    }

    // Method for accessing the search API
    public void getImages(final String query, JsonHttpResponseHandler handler) {
        try {
            String url = getApiUrl("/v2/search/image?query=");
            client.addHeader("Authorization",API_KEY);
            client.get(url + URLEncoder.encode(query, "utf-8"), handler);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    }
}
